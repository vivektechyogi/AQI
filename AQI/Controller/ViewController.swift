...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        chartPopupView.isHidden = false
        closeButon.addRadius(brRadius: 16)
                ...
                if let aqisData = aqis{
                    for aqi in aqisData{
                        ...
                        if self.selectedCity == aqi.city ?? "" {
                            if self.graphData.count == 0 || (Date().timeIntervalSince1970 - Array(self.graphData.keys).max()! >= 18){
                                self.graphData.updateValue(aqi.aqi ?? 0.0, forKey: Date().timeIntervalSince1970)
                            }
...
extension ViewController:UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let data = aqiData[Array(aqiData.keys)[indexPath.row]]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        selectedCity = Array(aqiData.keys)[indexPath.row]
        chartPopupView.isHidden = false
        updatePopupView(data:  aqiData[Array(aqiData.keys)[indexPath.row]]!)
    }
}
